...
loadData <- function(chr, start, end, cnvID, sampleIDs,coveragefile,medianfile,bins,newsizefilter,SampleExcludeList, SampleIncludeList,poorbincov=NULL,raw_cov=NULL)
  {
    if (is.null(raw_cov)) {
      if (end - start > newsizefilter) {
        ...
        cov1 <- read.table(pipe(paste("bedtools merge -i ", pointFile, " | tabix -h ",coveragefile," -R - | sed 's/^#//'|sed 's/Start/start/g'|sed 's/Chr/chr/g'|sed 's/End/end/g'", sep = "")),sep = "\t", header = TRUE, check.names = FALSE)
